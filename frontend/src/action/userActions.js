...
export const register = (name, email, password, isFarmer) => async (dispatch) => {
    try {
      dispatch({
      ...
      const { data } = await axios.post(
        '/api/users',
        {
          ...
          email,
          password,
          isFarmer, // Include isFarmer in the registration request
        },
        config
      );
...
export const updateUserProfile=(dt)=> async (dispatch,getState)=>{
    try {
        ...
        const {data} = await axios.put(`/api/users/profile`,dt,
            config
        )
